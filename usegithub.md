...
5.测试一下我们的设置是否正确
  ...
  输入yes
  我本机显示连接关闭，然后我继续执行上诉命令，提示输入passphrase（密码），输入前面自己设置的passphrase。回车登录成功登录成功登录成功登录成功！

6：在本地设置Git信息 
6.1设置用户名和邮箱 
  $ git config --global user.name "Firstname Lastname" 
  $ git config --global user.email "your_email@youremail.com" 此处用户名为自己的实际姓名(自定义的)，而非登录用户名 
